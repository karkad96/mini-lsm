...
#[cfg(test)]
mod wal_tests {
    use mini_lsm::{Op, Record, RecordError, SyncMode, Wal};
    use std::io::Write;
    ...
    // --- SyncMode / with_sync_mode() ---

    #[test]
    fn batch_only_mode_recovers_correctly() {
        let path = tmp_path();
        let mut wal = Wal::with_sync_mode(&path, SyncMode::BatchOnly).unwrap();
    ...
    fn never_mode_recovers_correctly() {
        let path = tmp_path();
        let mut wal = Wal::with_sync_mode(&path, SyncMode::Never).unwrap();
        let recs = vec![put(b"x", b"1"), del(b"y")];
        for r in &recs {
            wal.append(r).unwrap();
        }
    ...
    fn sync_mode_does_not_affect_recovered_data() {
        let modes = [SyncMode::Always, SyncMode::BatchOnly, SyncMode::Never];
    ...
    fn iter_yields_same_records_as_recover() {
        ...
        let from_iter: Vec<Record> = Wal::iter(&path)
            .unwrap()
            .map(|r| r.unwrap())
            .collect();
    ...
    fn iter_on_empty_file_yields_nothing() {
        let path = tmp_path();
        Wal::new(&path).unwrap();
        assert_eq!(Wal::iter(&path).unwrap().count(), 0);
        std::fs::remove_file(&path).ok();
    }
    ...
    fn iter_stops_at_truncated_record() {
        ...
        let records: Vec<Record> = Wal::iter(&path).unwrap().map(|r| r.unwrap()).collect();
    ...
    fn iter_surfaces_crc_error() {
        ...
        *data.last_mut().unwrap() ^= 0xFF;
        ...
        let result: Vec<Result<Record, _>> = Wal::iter(&path).unwrap().collect();
